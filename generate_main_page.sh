...
append_body () {
	...
	((EC += $?))
        ...
        echo -e "\t\t<h3><u>Source Code</u></h3>\n\t\t<p>\n\t\t\tAeolus Logger is entirely written in BASH and the source code can be found in the following Git repository: <a href=\"https://github.com/gzachos/aeolus-logger\" target=\"_blank\">[Aeolus Logger - GitHub Repository]</a>\n\t\t</p>\n\t\t<br>" >> ${TMP_FILE}
        ((EC += $?))
