...
function append_variable_section () {
        ...
        STRING="$STRING\t\t\t<tr>\n\t\t\t\t<td>${DATES_TIMES_U_ARRAY[$INDEX]}</td>\n\t\t\t\t<td>${VALUES_U_ARRAY[$INDEX]}</td>\n\t\t\t\t<td>${VALUES_S_ARRAY[$INDEX]}</td>\n\t\t\t\t<td>${VALUES_HU_ARRAY[$INDEX]}</td>\n\t\t\t\t<td>${VALUES_HS_ARRAY[$INDEX]}</td>\n\t\t\t</tr>\n"
        echo -e "$STRING" >> /var/www/html/emerson_$1/measurement_report_$1.html
}
...
function init_temp_unit_time () {
	DATES_TIMES_U=$(grep -i time /var/www/html/emerson_$1/data/temp/temp_unit.txt | tr -d "</time>" | tr -d "\r")
	INDEX=0
...
function init_temp_sys_time () {
	DATES_TIMES_S=$(grep -i time /var/www/html/emerson_$1/data/temp/temp_sys.txt | tr -d "</time>" | tr -d "\r")
	INDEX=0
...
function init_temp_unit_value () {
        VALUES_U=$(grep -i value /var/www/html/emerson_$1/data/temp/temp_unit.txt | tr -d "</value>" | tr -d "\r")
        INDEX=0
        for x in $VALUES_U
        do
...
function init_temp_sys_value () {
        VALUES_S=$(grep -i value /var/www/html/emerson_$1/data/temp/temp_sys.txt | tr -d "</value>" | tr -d "\r")
        INDEX=0
        for x in $VALUES_S
        do
...
function init_hum_unit_time () {
	DATES_TIMES_HU=$(grep -i time /var/www/html/emerson_$1/data/hum/hum_unit.txt | tr -d "</time>" | tr -d "\r")
	INDEX=0
...
function init_hum_sys_time () {
	DATES_TIMES_HS=$(grep -i time /var/www/html/emerson_$1/data/hum/hum_sys.txt | tr -d "</time>" | tr -d "\r")
	INDEX=0
...
function init_hum_unit_value () {
        VALUES_HU=$(grep -i value /var/www/html/emerson_$1/data/hum/hum_unit.txt | tr -d "</value>" | tr -d "\r")
        INDEX=0
        for x in $VALUES_HU
        do
...
function init_hum_sys_value () {
        VALUES_HS=$(grep -i value /var/www/html/emerson_$1/data/hum/hum_sys.txt | tr -d "</value>" | tr -d "\r")
        INDEX=0
        for x in $VALUES_HS
        do
