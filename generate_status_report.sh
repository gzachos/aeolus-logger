...
init_values () {
	...
	then
		...
		then
			...
			if [ "${MINUTES}" -eq "00" ]
			then
				echo "System temperature at Emerson #${1} has reached ${VALUES_ARRAY[1]} degrees Celsius!" > /tmp/message_body.txt
				echo "" >> /tmp/message_body.txt
				echo "This message was automatically generated by Aeolus Logger." >> /tmp/message_body.txt
				if [ "${LEAP_HOUR}" -eq "0" ]
				then
					mail -s "[AEOLUS] High Temperature at Emerson #${1}" support@cse.uoi.gr -aFrom:Aeolus\ Root\<root@aeolus.cs.uoi.gr\> < /tmp/message_body.txt
				else
					mail -s "[AEOLUS] High Temperature at Emerson #${1}" gzachos@cse.uoi.gr -aFrom:Aeolus\ Root\<root@aeolus.cs.uoi.gr\> < /tmp/message_body.txt
				fi
			fi
		fi
	fi
